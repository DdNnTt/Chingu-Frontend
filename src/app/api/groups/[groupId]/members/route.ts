...
export async function GET(
  ...
  try {
    ...
    console.log('[그룹 멤버 프록시] 백엔드 응답 텍스트:', text);

    if (!res.ok) {
      let errorMessage = '그룹 멤버 조회 실패';
      try {
    ...
    try {
      ...
      console.log('[그룹 멤버 프록시] 파싱된 응답 데이터:', data);
      return NextResponse.json(data, { status: res.status });
    } catch (parseError) {
      console.error('[그룹 멤버 프록시] JSON 파싱 실패:', parseError);
      return NextResponse.json({ message: '응답 파싱 실패' }, { status: 500 });
    }
  } catch (err) {
    console.error('[그룹 멤버 프록시 오류]', err);
