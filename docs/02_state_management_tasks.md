...
## A. 대상 컴포넌트 리스트 업 (먼저 1회 수행)

> 여러 컴포넌트를 찾아 리스트 업한 뒤, **아래 B~5를 리스트 순서대로 한 컴포넌트씩** 적용한다.

- [ ] 상태 관리 리팩토링이 필요한 컴포넌트 전수 조사  
       (서버 데이터를 useState/fetch+useEffect로 쓰는 페이지, 파생 상태를 useState로 쓰는 컴포넌트 등)
...
- **현재 Primary**: 리스트의 **N번째** 컴포넌트를 선택한 뒤, 아래 0→5 단계를 해당 컴포넌트만 대상으로 수행한다.
- 한 컴포넌트의 5.Gate까지 끝나면 체크하고, 다음 순번으로 넘어가서 다시 0부터 진행.
